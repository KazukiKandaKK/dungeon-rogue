...
function _buildBase() {
  ...
  // 街の小動物プール（拠点描画専用。歩行可能タイルだけに置かれる）
  ambientCreatures = createAmbientCreatures(
    (tx, ty) => map.isWalkable(tx, ty), map.cols, map.rows,
  );

  // 拠点のインタラクト可能オブジェクト（ベンチ・井戸・焚き火・看板 等）
  baseInteractables = buildDefaultBaseInteractables();
...
function startLoop(canvas) {
  ...
  function loop(ts) {
    ...
    if (gamePhase === 'BASE' && ambientCreatures.length > 0 && map) {
      ...
      const phase = ((ts / 1000 / 240) % 1 + 1) % 1; // TIME_OF_DAY_CYCLE_MS=240s
      // dusk/dawn のピークで眠る判定。詳細ロジックは drawAmbientCreatures 側でも見る
      const nightFactor =
        (phase < 0.1 || phase > 0.85) ? 1.0 :
        (phase < 0.2)                  ? (0.2 - phase) / 0.1 :
        (phase > 0.65)                 ? (phase - 0.65) / 0.20 : 0;
      updateAmbientCreatures(
        ambientCreatures, dt,
        (tx, ty) => map.isWalkable(tx, ty),
        map.cols, map.rows, nightFactor,
      );
    }
...
function _handleBaseInput() {
  ...
  if (input.justPressed('Enter') || input.justPressed('KeyE')) {
    ...
    // ── 拠点の小物（ベンチ／井戸／焚き火／看板 等）との対話 ──
    const nowMs = Date.now();
    if (nowMs - _lastInteractTime >= 300) {
      const it = findInteractableNear(baseInteractables, player.tx, player.ty);
      if (it) {
        _lastInteractTime = nowMs;
        const line = it.flavor[Math.floor(Math.random() * it.flavor.length)] ?? '';
        if (line) logger.add(line, 'normal');
        if (it.onInteract) {
          it.onInteract({
            player,
            addLog: (msg, type) => logger.add(msg, type ?? 'normal'),
            now: nowMs,
          });
        }
        return;
...
function _draw(ctx, W, H, now) {
  ...
  if (gamePhase === 'BASE') {
    ...
    drawBaseInteractables(
      ctx, baseInteractables, camOffX, camOffY, TILE_SIZE, now,
