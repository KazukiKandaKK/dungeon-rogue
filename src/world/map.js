// ─────────────────────────────────────────────
// map.js  GameMap クラス（2D トップダウン描画・テーマ対応）
// ─────────────────────────────────────────────

import { TILE_DEF, TILE, TILE_SIZE, THEMES } from './tiles.js?v=11';
import { DungeonGenerator, BossArenaGenerator, BaseRoomGenerator } from './dungeon.js?v=13';
...
export class GameMap {
   ...
   * @param {number} cols
   * @param {number} rows
   * @param {'dungeon'|'forest'|'town'} theme
   * @param {boolean} bossArena
   */
  constructor(cols, rows, theme = 'dungeon', bossArena = false, trapDensity = 1) {
    this.cols        = cols;
    this.rows        = rows;
  ...
  _makeGenerator(cols, rows, theme) {
  ...
  getExitDir(tx, ty) {
    for (const [dir, pos] of Object.entries(this.exits)) {
      if (pos.tx === tx && pos.ty === ty) return dir;
    }
    return null;
  }
   ...
   * 2D トップダウン描画
   */
  draw(ctx, camOffX, camOffY, now = 0, exploredTiles = null) {
  ...
  // ─── 1タイル描画（テーマ別） ───────────────────
  _drawTile(ctx, sx, sy, tileId, tx, ty) {
  ...
  _drawDungeonTile(ctx, sx, sy, tileId, th, ts, tx, ty) {
    ...
    if (tileId === TILE.WALL) {
      ...
      if (seed % 7 === 0) {
        ...
        ctx.lineWidth   = 0.9;
        const sc = ts / 32;
        const cx = sx + Math.round(ts * 0.12) + (seed % Math.round(ts * 0.18));
        const cy = sy + Math.round(ts * 0.1)  + ((seed * 5) % Math.round(ts * 0.22));
        ctx.beginPath();
        ctx.moveTo(cx,           cy);
        ctx.lineTo(cx + 4 * sc,  cy + 5 * sc);
        ctx.lineTo(cx + 2 * sc,  cy + 9 * sc);
        ctx.lineTo(cx + 5 * sc,  cy + 13 * sc);
        ctx.stroke();
        ctx.restore();
      }
  ...
  _drawForestTile(ctx, sx, sy, tileId, tx, ty, th, ts) {
    if (tileId === TILE.PILLAR || tileId === TILE.TRAP || tileId === TILE.WATER) {
      tileId = TILE.FLOOR;
    }
    if (tileId === TILE.WALL) {
      ...
      ctx.beginPath();
      ctx.ellipse(cx, cy - ts * 0.08, r1, r1 * 0.82, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      if (seed > 25) {
        ...
        ctx.beginPath();
        ctx.ellipse(cx - ts * 0.2, cy - ts * 0.1, r1 * 0.68, r1 * 0.58, -0.3, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(cx + ts * 0.18, cy - ts * 0.05, r1 * 0.62, r1 * 0.54, 0.3, 0, Math.PI * 2);
        ctx.fill();
      }
      ...
      ctx.beginPath();
      ctx.ellipse(cx - ts * 0.08, cy - ts * 0.18, r1 * 0.4, r1 * 0.3, -0.4, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
      ...
      ctx.fillRect(cx - trunkW / 2, sy + ts - trunkH, trunkW, trunkH);
      // 幹ハイライト
      ctx.save();
      ctx.globalAlpha = 0.2;
      ctx.fillStyle = '#8b5020';
      ctx.fillRect(cx - trunkW / 2 + 1, sy + ts - trunkH + 2, Math.round(trunkW * 0.4), trunkH - 4);
      ctx.restore();
  ...
  _drawTownTile(ctx, sx, sy, tileId, tx, ty, th, ts) {
    if (tileId === TILE.PILLAR || tileId === TILE.TRAP || tileId === TILE.WATER) {
      tileId = TILE.FLOOR;
    }
    if (tileId === TILE.WALL) {
      ...
      // ── レンガ目地（ts比例） ──────────────────────────────────────
      const bh = Math.round(ts / 4);  // レンガ高さ（48pxなら12px）
      const bw = Math.round(ts / 2);  // レンガ幅（48pxなら24px）
  ...
  // ─── 障害物描画 ────────────────────────────────
  _drawObstacles(ctx, camOffX, camOffY, now) {
    const ts = TILE_SIZE;
    const W  = ctx.canvas.width;
    const H  = ctx.canvas.height;
  ...
  _drawWater(ctx, sx, sy, ts, tx, ty, now) {
    ctx.save();
  ...
  _drawRevealedTrap(ctx, sx, sy, ts) {
    ctx.save();
    const cx = sx + ts / 2, cy = sy + ts / 2;
    // 穴の影
  ...
  // ─── 階段（アニメ付きベクター）────────────────
  _drawStairs(ctx, camOffX, camOffY, now) {
  ...
  // ─── エグジット矢印（アニメ風グロウ）─────────
  _drawExitArrows(ctx, camOffX, camOffY, now) {
