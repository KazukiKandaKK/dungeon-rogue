// ─────────────────────────────────────────────
// forest.ts  森マップ生成（セルオートマトン）
// ─────────────────────────────────────────────

import { TILE } from './tiles.js';
import type { TileType } from './tiles.js';

export interface ExitPos { tx: number; ty: number; }
export type ExitMap = Partial<Record<'N' | 'S' | 'E' | 'W', ExitPos>>;
...
export class ForestGenerator {
  ...
  generate(): ForestResult {
    // 1. ランダムノイズ（52% ツリー → やや密な森）
    let grid: TileType[][] = Array.from({ length: this.rows }, (_, y) =>
  ...
  private _smooth(grid: TileType[][]): TileType[][] {
  ...
  private _countWallNeighbors(grid: TileType[][], x: number, y: number): number {
    let n = 0;
  ...
  private _clearArea(grid: TileType[][], cx: number, cy: number, r: number): void {
  ...
  private _addExits(grid: TileType[][]): ExitMap {
    const exits: ExitMap = {};
    type DirKey = 'N' | 'S' | 'W' | 'E';
    const dirs: Record<DirKey, { scanX: number; scanY: number; isH: boolean }> = {
      N: { scanY: 1,               scanX: Math.floor(this.cols / 2), isH: true  },
      S: { scanY: this.rows - 2,   scanX: Math.floor(this.cols / 2), isH: true  },
      W: { scanY: Math.floor(this.rows / 2), scanX: 1,              isH: false },
      E: { scanY: Math.floor(this.rows / 2), scanX: this.cols - 2,  isH: false },
    };

    for (const _dir of Object.keys(dirs) as DirKey[]) {
      const dir = _dir;
      const cfg = dirs[dir];
  ...
  private _findWalkableNear(
    grid: TileType[][], cx: number, cy: number, dir: string
  ...
  // 中央から遠い歩行可能タイルに階段を配置
  private _placeStairs(
    grid: TileType[][], spawnX: number, spawnY: number, exits: ExitMap
  ): ExitPos | null {
    const isExit = (tx: number, ty: number) =>
      Object.values(exits).some(e => e && e.tx === tx && e.ty === ty);
    const minDist = Math.floor(Math.min(this.cols, this.rows) * 0.4);

    interface Candidate { tx: number; ty: number; dist?: number; }
